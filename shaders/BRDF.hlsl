...
/**
Optimizied Schlick
\see http://seblagarde.wordpress.com/2011/08/17/hello-world/
*/
...
float3 BRDF(float3 L, float3 V, float3 N, float3 cdiff, float3 cspec, float roughness)
{
    ...
    // specular term
    float3 Rs = cspec/M_PI * (F*G*D) / (4*NoL*NoV);
